...
class Interfejs:
    ...
    def przycisk(self, x, y, w, h, napis, f, color="lightCyan3"):
        return Button(self.root, text=napis, command=f, bg=color).place(
    ...
    def next(self):
        try:
            ...
            self.canvas.create_image(
                0, 0, image=self.photos[self.liczba % 2], anchor=NW
            )
...
class Wykres:
    def __init__(self, interfejs):
        ...
        if raty == "stałe":
            plt.plot(
                list(czas) + [(okres + 1) / 12],
                zadluzenia,
                label="Raty:{}".format(round(rat, 2)),
            )
        else:
            plt.plot(
                list(czas) + [(okres + 1) / 12],
                zadluzenia,
                label="Pierwsza rata:{}, Ostatnia rata:{}".format(
                    round(rata[0], 2), round(rata[-1], 2)
                ),
            )

        plt.title(
            "Odsetki:{}zł  ;  Do spłaty:{}zł".format(
                round(suma, 2), round(kredyt + suma + dodatek, 2)
            )
        )
        ...
        interfejs.canvas.create_image(
            0, 0, image=interfejs.photos[interfejs.liczba % 2], anchor=NW
        )
